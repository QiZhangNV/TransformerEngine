...
void nvte_cutlass_grouped_gemm(const NVTETensor *A, const NVTETensor *B, NVTETensor *D,
  ...
  const transformer_engine::Tensor *inputA =
      reinterpret_cast<const transformer_engine::Tensor *>(A[0]);
  if (transa) {
  ...
  const transformer_engine::Tensor *outputD =
      reinterpret_cast<const transformer_engine::Tensor *>(D[0]);
  ...
  const int gemm_n =
      transb ? reinterpret_cast<const transformer_engine::Tensor *>(B[0])->flat_first_dim()
             : reinterpret_cast<const transformer_engine::Tensor *>(B[0])->flat_last_dim();
  //   printf("num_gemms: %d\n", num_gemms);
  ...
  if (transb) {
    generic_moe_gemm_kernelLauncher<__nv_fp8_e4m3, __nv_fp8_e8m0, __nv_fp8_e4m3, __nv_fp8_e8m0,
        ...
        gemm_n,    // gemm_n
        gemm_k,    // gemm_k
        num_gemms, reinterpret_cast<transformer_engine::Tensor *>(workspace[0])->data.dptr, stream);
  } else {
    generic_moe_gemm_kernelLauncher<__nv_fp8_e4m3, __nv_fp8_e8m0, __nv_fp8_e4m3, __nv_fp8_e8m0,
                                    half, false>(
        inputA_ptr, inputA_SF_ptr, inputB_ptr_list, inputB_SF_ptr_list, outputD_ptr,
...
void nvte_cutlass_grouped_gemm_wgrad(const NVTETensor *A, const NVTETensor *B, NVTETensor *D,
  ...
  // Process B
  const transformer_engine::Tensor *inputB =
      reinterpret_cast<const transformer_engine::Tensor *>(B[0]);
  if (transb) {
    NVTE_CHECK(inputB->has_data(), "Input B is missing row-wise usage");
  } else {
    NVTE_CHECK(inputB->has_columnwise_data(), "Input B is missing column-wise usage");
  ...
  for (size_t i = 0; i < num_gemms; i++) {
    const transformer_engine::Tensor *outputD =
        reinterpret_cast<const transformer_engine::Tensor *>(D[i]);
    NVTE_CHECK(outputD->has_data(), "Input D is missing row-wise usage");
  ...
  if (transD) {
    if (accumulate) {
      generic_moe_gemm_wgrad_kernelLauncher<__nv_fp8_e4m3, __nv_fp8_e8m0, __nv_fp8_e4m3,
          ...
          reinterpret_cast<transformer_engine::Tensor *>(workspace[0])->data.dptr, stream);
    } else {
      generic_moe_gemm_wgrad_kernelLauncher<__nv_fp8_e4m3, __nv_fp8_e8m0, __nv_fp8_e4m3,
