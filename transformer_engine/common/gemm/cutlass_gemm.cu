...
//   printf("grad: %d\n", grad);
  ...
  // Process A
  const transformer_engine::Tensor *inputA = reinterpret_cast<const transformer_engine::Tensor *>(A[0]);
  if (transa) {
  ...
  for (size_t i = 0; i < num_gemms; i++) {
      const transformer_engine::Tensor *inputB = reinterpret_cast<const transformer_engine::Tensor *>(B[i]);
      if (transb) {
  ...
  // Process D
  const transformer_engine::Tensor *outputD = reinterpret_cast<const transformer_engine::Tensor *>(D[0]);
  ...
  const int gemm_n = transb ? reinterpret_cast<const transformer_engine::Tensor *>(B[0])->flat_first_dim() : reinterpret_cast<const transformer_engine::Tensor *>(B[0])->flat_last_dim(); 
//   printf("num_gemms: %d\n", num_gemms);
...
//   printf("Calling cutlass gemm\n");
  if (transb) {
        generic_moe_gemm_kernelLauncher<__nv_fp8_e4m3, __nv_fp8_e8m0, __nv_fp8_e4m3, __nv_fp8_e8m0, half, true>(
                                ...
                                gemm_k,                // gemm_k
                                num_gemms,
                                reinterpret_cast<transformer_engine::Tensor *>(workspace[0])->data.dptr,
                                stream);
  }
  else {
...
//   printf("accumulate: %d\n", accumulate);
  ...
  // Process B
  const transformer_engine::Tensor *inputB = reinterpret_cast<const transformer_engine::Tensor *>(B[0]);
  if (transb) {
    NVTE_CHECK(inputB->has_data(), "Input B is missing row-wise usage");
  } else {
    NVTE_CHECK(inputB->has_columnwise_data(), "Input B is missing column-wise usage");
  ...
  for (size_t i = 0; i < num_gemms; i++) {
    const transformer_engine::Tensor *outputD = reinterpret_cast<const transformer_engine::Tensor *>(D[i]);
    NVTE_CHECK(outputD->has_data(), "Input D is missing row-wise usage");
